...
public class RecordButton extends Button {
	...
	private void showVoiceDialog(int flag) {
		...
		switch (flag) {
		case 1:
			dialogImg.setImageResource(R.drawable.rc_cancel_send_voice);
			dialogTextView.setText("松开手指，取消发送");
			break;

		default:
			dialogImg.setImageResource(R.drawable.rc_volume_zero);
			dialogTextView.setText("手指上划，取消发送");
			break;
		}
		mRecordDialog.show();
	}
	...
	// 录音Dialog图片随录音音量大小切换
		private void setDialogImage() {
			if (voiceValue < 1000.0) {
				dialogImg.setImageResource(R.drawable.rc_volume_zero);
			} else if (voiceValue > 1000.0 && voiceValue < 2500.0) {
				dialogImg.setImageResource(R.drawable.rc_volume_one);
			} else if (voiceValue > 2500.0 && voiceValue < 4000.0) {
				dialogImg.setImageResource(R.drawable.rc_volume_two);
			} else if (voiceValue > 4000.0 && voiceValue < 6000.0) {
				dialogImg.setImageResource(R.drawable.rc_volume_three);
			} else if (voiceValue > 6000.0) {
				dialogImg.setImageResource(R.drawable.rc_volume_four);
			} 
		}
			...
			if (recordState == RECORD_ON) {
				...
				if (isCanceled) {
					...
					if (recodeTime < MIN_RECORD_TIME) {
						...
						if (listener != null) {
							//录音消息的回调发送
							listener.sendVoice(mAudioRecorder.getVoicePath()+ mAudioRecorder.getFileName() + ".amr",(int) recodeTime);
						}
					}
				}
				isCanceled = false;
